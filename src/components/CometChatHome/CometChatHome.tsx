...
import transferOwnership from "../../assets/transferOwnership.svg";
import deleteGroupIcon from "../../assets/deleteGroup.svg";
...
import { CometChatGroupDetails } from "../CometChatDetails/CometChatGroupDetails";
...
import { CometChatEmptyStateView } from "../CometChatEmptyState/CometChatEmptyStateView";
import { IGroupMemberBanned, IGroupMemberKicked, IGroupMemberUnBanned } from "@cometchat/chat-uikit-react";
import { AppContext } from "../../context/AppContext";
import { CometChatCallLogs } from "../CometChatCallLogs/CometChatCallLogs";
import { CometChatCallDetails } from "../CometChatCallLog/CometChatCallDetails";
import { CallLog, CometChatUIKitCalls } from "@cometchat/calls-sdk-javascript";
import { CometChatTabs } from "../CometChatSelector/CometChatTabs";
import {
    CometChatMessages, CometChatMessageComposer, CometChatMessageHeader, CometChatMessageList, CometChatUsers, CometChatGroups, CometChatConversations, CometChatUIKit, CometChatUserEvents, CometChatIncomingCall, CometChatUIEvents, CometChatGroupEvents, CometChatConversationEvents, CometChatMessageEvents, CometChatToast, CometChatButton, CometChatAlertPopup, CometChatUIKitConstants, IGroupMemberJoined, IMessages, MessageStatus, CometChatOption, getLocalizedString, CometChatUIKitLoginListener, States
} from "@cometchat/chat-uikit-react";
import UserProfile from "../ClerkAuth/UserProfile";
import { ChatRequestManager, sendChatRequest } from '../ChatRequests/ChatRequestManager';
...
function CometChatHome(props: { theme?: string }) {
   ...
   */
            ...
            <>
                {activeTab === "requests" ? (
                    ...
                    <>
                        ...
                        {selectedItem && <CometChatMessagesViewComponent />}
                        {!selectedItem && !showNewChat && <div className="cometchat-no-chat" />}
                    </>
                )}
            </>
        );
    ...
    const onSelectorItemClicked = (e: Conversation | User | Group | Call, type: string) => {
        ...
        // For user requests - Send chat request instead of direct chat
        if (type === "updateSelectedItemUser" && e instanceof CometChat.User) {
            // Send chat request
            sendChatRequest(e as CometChat.User).then(success => {
                if (success) {
                    // Show notification to the user
                    toastTextRef.current = "Chat request sent";
                    setShowToast(true);
                    
                    // Don't update UI to show chat - wait for acceptance
                    return;
                }
            });
            return;
        ...
        // For other types (groups, conversations) proceed as normal
        if (e instanceof Group && activeTab === "groups") {
            if (e.getType() === CometChat.GROUP_TYPE.PASSWORD) {
                showJoinGroupRef.current = true;
            }
        } else {
            setAppState({ type: "updateSideComponent", payload: { visible: false, type: "" } });
