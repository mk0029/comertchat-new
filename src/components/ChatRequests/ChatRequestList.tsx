...
import { CometChatListItem, CometChatButton, CometChatAvatar, getLocalizedString } from "@cometchat/chat-uikit-react";
...
const ChatRequestList: React.FC<ChatRequestListProps> = ({ onItemClick }) => {
  ...
  const pendingRequests = appState.pendingRequests.filter(req => req.status === "pending");
  ...
  const handleAccept = async (request: any) => {
    try {
      ...
      if (request.type === "user") {
        ...
        setAppState({ type: "updateSelectedItemUser", payload: request.sender });
  ...
  return (
    <div className="chat-requests-list">
      {pendingRequests.map((request, index) => (
        <div key={`${request.sender.getUid()}-${index}`} className="chat-request-item">
          <CometChatListItem
            ...
            subtitleView={
              <div className="chat-request-subtitle">
                {request.type === "user" 
                  ? getLocalizedString("wants_to_chat") || "wants to chat with you" 
                  : getLocalizedString("added_you_to_group") || "added you to a group"}
              </div>
            }
          />
